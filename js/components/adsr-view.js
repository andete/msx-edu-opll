...
// attenuation. It shows the Key-On "damp" as a dip before the attack climbs —
// the OPLL quirk that has no PSG/SCC counterpart. See reference §7.
...
export class AdsrView {
  ...
  draw(env, live) {
    ...
    // --- schematic geometry -------------------------------------------------
    // segment widths weighted by (16 − field): faster rates draw narrower.
    const wA = 16 - env.ar, wD = 16 - env.dr, wS = 10, wR = 16 - env.rr;
    const totalW = Math.max(1, wA + wD + wS + wR);
    const span = x1 - x0;
    const xA = x0 + (wA / totalW) * span;
    const xD = xA + (wD / totalW) * span;
    const xS = xD + (wS / totalW) * span;
    const xR = x1;
    ...
    ctx.lineTo(xS, ampY(susDisplay));        // sustain hold
    ...
    label(ctx, 'S', xD, xS, h - 3);
    label(ctx, 'R', xS, xR, h - 3);
    ...
    if (live && live.state !== IDLE) {
      ...
      switch (live.state) {
        ...
        case RELEASE: x = xS + (xR - xS) * Math.min(1, live.eg / EG_MUTE); break;
        default: x = x0;
      }
      ...
      ctx.fillText(STATE_NAMES[live.state], 8, 14); ctx.globalAlpha = 1;
    }
  }
}
